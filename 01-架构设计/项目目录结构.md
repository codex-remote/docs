...
项目采用五个并列、互不嵌套的应用源码仓库，另有 Runtime 分发、第三方 Homebrew Tap 和文档仓库。父目录只是本地工作区，不是 Monorepo。GitHub 组织还预留了 `releases`、`protocol` 和 `codex-plugin` 等公开交付边界，它们不构成新的应用源码仓库。
...
├── homebrew-tap/               # 第三方 Homebrew Formula
