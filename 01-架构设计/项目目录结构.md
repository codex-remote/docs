...
`runtime-distribution` 只消费 Relay、Mac Agent 和 Mobile Web 的版本化构建输入，拥有 `codex-remote` CLI、Supervisor、Manifest、确定性组装和安装测试。`homebrew-tap` 只保存第三方 Formula 元数据。未来官方 Homebrew 分发同样只引用公开的签名、公证 Runtime 资产。这些分发边界不得导入兄弟仓库源码；跨仓复用必须通过版本化契约或构建产物完成。
