...
项目采用五个并列、互不嵌套的应用源码仓库，另有 Runtime 分发、Homebrew Tap 和文档仓库。父目录只是本地工作区，不是 Monorepo。
...
`runtime-distribution` 只消费 Relay、Mac Agent 和 Mobile Web 的版本化构建输入，拥有 `codex-remote` CLI、Supervisor、Manifest、确定性组装和安装测试。`homebrew-tap` 只保存 Formula 元数据。两者都不得导入兄弟仓库源码或公开私有源码。
...
| `homebrew-tap` | Formula revision | Formula 元数据 |
